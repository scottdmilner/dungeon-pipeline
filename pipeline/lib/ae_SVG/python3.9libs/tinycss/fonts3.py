...
class CSSFonts3Parser(CSS21Parser):
    ...
    def parse_at_rule(self, rule, previous_rules, errors, context):
        if rule.at_keyword == "@font-face":
            ...
            return FontFaceRule(rule.at_keyword, declarations, rule.line, rule.column)
        elif rule.at_keyword == "@font-feature-values":
            family_names = tuple(self.parse_font_feature_values_family_names(rule.head))
            at_rules, body_errors = self.parse_rules(
                rule.body or [], "@font-feature-values"
            )
            errors.extend(body_errors)
            return FontFeatureValuesRule(
                rule.at_keyword, at_rules, family_names, rule.line, rule.column
            )
        elif rule.at_keyword in self.FONT_FEATURE_VALUES_AT_KEYWORDS:
            if context != "@font-feature-values":
                raise ParseError(
                    rule, "{0} rule not allowed in {1}".format(rule.at_keyword, context)
                )
            declarations, body_errors = self.parse_declaration_list(rule.body)
            errors.extend(body_errors)
    ...
    def parse_font_feature_values_family_names(self, tokens):
        ...
        for token in tokens:
            if token.type == "DELIM" and token.value == "," and family:
                ...
                family = ""
                current_string = False
            elif token.type == "STRING" and not family and (current_string is False):
                family = token.value
                current_string = True
